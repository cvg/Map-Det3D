...
def export(
    ...
    label_map = None
    if not test_mode:
        if not (label_map_file and os.path.isfile(label_map_file)):
            raise FileNotFoundError(
                "label_map_file is required for labeled export: "
                f"{label_map_file}"
            )
        if scannet200:
    ...
    aligned_mesh_vertices = np.concatenate(
        [pts[:, 0:3], mesh_vertices[:, 3:]], axis=1
    )
    ...
    if not test_mode:
        ...
        instance_ids = np.zeros(
            shape=(num_verts), dtype=np.uint32
        )  # 0: unannotated
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "path to scannetv2-labels.combined.tsv. Required unless "
            "--test_mode/--mesh_only is set."
        ),
    )
    parser.add_argument(
        "--scannet200",
    ...
    seg_file = os.path.join(
        opt.scan_path, scan_name + "_vh_clean_2.0.010000.segs.json"
    )
    meta_file = os.path.join(
        opt.scan_path, scan_name + ".txt"
