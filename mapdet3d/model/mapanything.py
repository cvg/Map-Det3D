...
class MapAnything(PyTorchModelHubMixin, nn.Module):
    ...
    def __init__(
        ...
        self.load_specific_pretrained_submodules = (
        ...
        self.info_sharing_return_type = info_sharing_config[
            "model_return_type"
        ]
        ...
        if self.encoder_config["uses_torch_hub"]:
            self.encoder_config["torch_hub_force_reload"] = (
        ...
        ray_dirs_encoder_config = self.geometric_input_config[
        ...
        depth_encoder_config = self.geometric_input_config[
        ...
        depth_scale_encoder_config = self.geometric_input_config[
            "scale_encoder_config"
        ]
        depth_scale_encoder_config["enc_embed_dim"] = (
            self.encoder.enc_embed_dim
        )
        self.depth_scale_encoder = encoder_factory(
            **depth_scale_encoder_config
        )

        # Initialize the encoder for camera rotation
        cam_rot_encoder_config = self.geometric_input_config[
        ...
        cam_trans_scale_encoder_config["enc_embed_dim"] = (
            self.encoder.enc_embed_dim
        )
        self.cam_trans_scale_encoder = encoder_factory(
            **cam_trans_scale_encoder_config
        )
    ...
    def _initialize_info_sharing(self, info_sharing_config):
        ...
        custom_positional_encoding = info_sharing_config[
    ...
    def _initialize_prediction_heads(self, pred_head_config):
        ...
        if self.pred_head_type == "linear":
            ...
            pred_head_config["regressor_head"]["input_feature_dim"] = (
                pred_head_config["feature_head"]["feature_dim"]
            )
        ...
        if self.pred_head_type == "linear":
            ...
            self.dpt_feature_head = DPTFeature(
                **pred_head_config["feature_head"]
            )
    ...
    def _initialize_adaptors(self, pred_head_config):
        ...
        if pred_head_config["adaptor_type"] == "pointmap":
            ...
            self.dense_adaptor = RayMapPlusDepthAdaptor(
                **pred_head_config["adaptor"]
            )
            ...
            pred_head_config["adaptor_type"] == "raymap+depth+confidence+mask"
        ):
            ...
            pred_head_config["adaptor_type"] == "raydirs+depth+pose+confidence"
        ):
            assert (
                self.pred_head_type == "dpt+pose"
            ), "Ray directions + depth + pose can only be used as scene representation with dpt + pose head."
            ...
            self.dense_adaptor = PointMapAdaptor(
                **pred_head_config["dpt_adaptor"]
            )
            self.pose_adaptor = CamTranslationPlusQuatsAdaptor(
                **pred_head_config["pose_adaptor"]
    ...
    def _load_pretrained_weights(self):
        ...
        if self.pretrained_checkpoint_path is not None:
            if not self.load_specific_pretrained_submodules:
                ...
                ckpt = torch.load(
                    self.pretrained_checkpoint_path, weights_only=False
                )
    ...
    def _compute_pose_quats_and_trans_for_across_views_in_ref_view(
        ...
        Args:
            ...
            per_sample_cam_input_mask (Tensor): Tensor containing the per sample camera input mask.

        Returns:
            Tensor: A tensor containing the pose quats for all the views in the frame of the reference view 0. (batch_size_per_view * view, 4)
            Tensor: A tensor containing the pose trans for all the views in the frame of the reference view 0. (batch_size_per_view * view, 3)
            Tensor: A tensor containing the per sample camera input mask.
        """
        ...
        if len(pose_quats_non_ref_views) > 0:
            ...
            pose_quats_non_ref_views = torch.cat(
                pose_quats_non_ref_views, dim=0
            )
            pose_trans_non_ref_views = torch.cat(
                pose_trans_non_ref_views, dim=0
            )
    ...
    def _encode_and_fuse_ray_dirs(
        ...
        Args:
            ...
            all_encoder_features_across_views (Tensor): Tensor containing the encoded features for all N views.
            per_sample_ray_dirs_input_mask (Tensor): Tensor containing the per sample ray direction input mask.

        Returns:
            Tensor: A tensor containing the encoded features for all the views.
        """
        ...
        ray_dirs = ray_dirs.permute(
            0, 3, 1, 2
        ).contiguous()  # (B * V, 3, H, W)
        ...
        ray_dirs_features_across_views = (
            ray_dirs_features_across_views
            * per_sample_ray_dirs_input_mask.unsqueeze(-1)
            .unsqueeze(-1)
            .unsqueeze(-1)
        )
        all_encoder_features_across_views = (
    ...
    def _encode_and_fuse_depths(
        ...
        Args:
            ...
            batch_size_per_view (int): Batch size per view.
            all_encoder_features_across_views (Tensor): Tensor containing the encoded features for all N views.
            per_sample_depth_input_mask (Tensor): Tensor containing the per sample depth input mask.
        ...
        for view_idx in range(num_views):
            ...
            if (
                ...
                if use_sparse_depth:
                    ...
                    if num_to_zero > 0:
                        ...
                        indices_to_zero = torch.randperm(num_valid_pixels)[
                            :num_to_zero
                        ]
            ...
            metric_scale_depth_mask_list.append(
                metric_scale_mask_for_curr_view
            )
        ...
        depth_norm_factors = torch.cat(
            depth_norm_factors_list, dim=0
        )  # (B * V, )
        # Encode the depth norm factors using the log scale encoder for depth
        log_depth_norm_factors = torch.log(
            depth_norm_factors + 1e-8
        )  # (B * V, )
        ...
        depth_scale_features_across_views = (
            depth_scale_features_across_views
            * metric_scale_depth_mask.unsqueeze(-1)
        )
    ...
    def _encode_and_fuse_cam_quats_and_trans(
        ...
        Args:
            ...
            batch_size_per_view (int): Batch size per view.
            all_encoder_features_across_views (Tensor): Tensor containing the encoded features for all N views.
            pose_quats_across_views (Tensor): Tensor containing the pose quats for all the views in the frame of the reference view 0. (batch_size_per_view * view, 4)
            pose_trans_across_views (Tensor): Tensor containing the pose trans for all the views in the frame of the reference view 0. (batch_size_per_view * view, 3)
            per_sample_cam_input_mask (Tensor): Tensor containing the per sample camera input mask.
        ...
        pose_trans_norm_factors_across_views = (
            pose_trans_norm_factors.unsqueeze(-1).repeat(num_views, 1)
        )  # (B, ) -> (B * V, 1)
        ...
        pose_trans_scale_features_across_views = self.cam_trans_scale_encoder(
            EncoderGlobalRepInput(
                data=log_pose_trans_norm_factors_across_views
            )
        ).features
        ...
        all_encoder_features_across_views = (
            ...
            + pose_trans_scale_features_across_views.unsqueeze(-1).unsqueeze(
                -1
            )
        )
    ...
    def _encode_and_fuse_optional_geometric_inputs(
        ...
        Args:
            views (List[dict]): List of dictionaries containing the input views' images and instance information.
            all_encoder_features_across_views (List[Tensor]): List of tensors containing the encoded image features for all N views.

        Returns:
            List[Tensor]: A list containing the encoded features for all N views.
        """
        num_views = len(views)
        ...
        overall_geometric_input_mask = overall_geometric_input_mask.repeat(
            num_views
        )
        ...
        per_sample_depth_input_mask = per_sample_depth_input_mask.repeat(
            num_views
        )
        per_sample_depth_input_mask = (
        ...
        all_encoder_features_across_views = (
            self._encode_and_fuse_cam_quats_and_trans(
                views,
                num_views,
                batch_size_per_view,
                all_encoder_features_across_views,
        ...
        all_encoder_features_across_views = (
            all_encoder_features_across_views.permute(0, 2, 3, 1).contiguous()
        )
        all_encoder_features_across_views = self.fusion_norm_layer(
            all_encoder_features_across_views
        )
        all_encoder_features_across_views = (
            all_encoder_features_across_views.permute(0, 3, 1, 2).contiguous()
        )
    ...
    def _compute_adaptive_minibatch_size(
        ...
        if device.type == "cuda":
            ...
            available_memory = torch.cuda.mem_get_info()[
                0
        ...
        computed_minibatch_size = int(
            usable_memory / max_estimated_memory_per_sample
        )
    ...
    def downstream_dense_head(
        self,
        dense_head_inputs: Tensor | list[Tensor],
    ...
    def downstream_head(
        ...
        if memory_efficient_inference:
            ...
            pose_final_outputs_list = (
                [] if self.pred_head_type == "dpt+pose" else None
            )
            for batch_idx in range(num_batches):
                ...
                if self.pred_head_type == "linear":
                    dense_head_inputs_batch = dense_head_inputs[
                        start_idx:end_idx
                    ]
                elif self.pred_head_type in ["dpt", "dpt+pose"]:
                ...
                if self.pred_head_type == "dpt+pose":
                    pose_head_inputs_batch = dense_head_inputs[-1][
                        start_idx:end_idx
                    ]
                    pose_head_outputs_batch = self.pose_head(
                        PredictionHeadInput(
                            last_feature=pose_head_inputs_batch
                        )
                    )
            ...
            dense_pred_data_dict = {
                key: torch.cat(
                    [
                        getattr(output, key)
                        for output in dense_final_outputs_list
                    ],
                    dim=0,
                )
            ...
            if self.pred_head_type == "dpt+pose":
                ...
                pose_pred_data_dict = {
                    key: torch.cat(
                        [
                            getattr(output, key)
                            for output in pose_final_outputs_list
                        ],
                        dim=0,
                    )
                    for key in available_keys
    ...
    def forward(self, views, memory_efficient_inference: bool = False):
        ...
        Process:
            ...
            2. Encodes the optional geometric inputs (ray directions, depths,
            camera rotations, camera translations).
            3. Fuses the encoded features from the N input views and the
            optional geometric inputs using addition and normalization.
            4. Information sharing across the encoded features and a scale
            token using a multi-view attention transformer.
            5. Passes the final features from transformer through the
            prediction heads.
            6. Returns the processed final outputs for N views.

        Assumption:
            - All the input views and dense geometric inputs have the same
            image shape.

        Args:
            views (List[dict]): List of dictionaries containing the input
            views' images and instance information. Each dictionary should
            contain the following keys: "img" (tensor): Image tensor of shape
            (B, C, H, W). Input images must be normalized based on the data
            norm type of image encoder. "data_norm_type" (list):
            [model.encoder.data_norm_type] Optionally, each dictionary can also
            contain the following keys for the respective optional geometric
            inputs: "ray_directions_cam" (tensor): Ray directions in the local
            camera frame. Tensor of shape (B, H, W, 3). "depth_along_ray"
            (tensor): Depth along the ray. Tensor of shape (B, H, W, 1).
            "camera_pose_quats" (tensor): Camera pose quaternions. Tensor of
            shape (B, 4). Camera pose is opencv (RDF) cam2world transformation.
            "camera_pose_trans" (tensor): Camera pose translations. Tensor of
            shape (B, 3). Camera pose is opencv (RDF) cam2world transformation.
            "is_metric_scale" (tensor): Boolean tensor indicating whether the
            geometric inputs are in metric scale or not. Tensor of shape
            (B, 1). memory_efficient_inference (bool): Whether to use memory
            efficient inference or not. This runs the dense prediction head
            (the memory bottleneck) in a memory efficient manner. Default is
            False.
        ...
        if self.info_sharing_return_type == "no_intermediate_features":
            final_info_sharing_multi_view_feat = self.info_sharing(
                info_sharing_input
            )
        elif self.info_sharing_return_type == "intermediate_features":
            (
        ...
        if self.pred_head_type == "linear":
            ...
            if self.use_encoder_features_for_dpt:
                ...
                stacked_intermediate_features_3 = torch.cat(
                    intermediate_info_sharing_multi_view_feat[2].features,
                    dim=0,
                )
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            if self.scene_rep_type in [
                ...
                output_pts3d = (
                    output_ray_origins
                    + output_ray_directions * output_depth_along_ray
                )
                # Split the predicted quantities back to their respective views
                output_ray_origins_per_view = output_ray_origins.chunk(
                    num_views, dim=0
                )
                    ...
                    res.append(
                        {
                            ...
                            * scale_final_output.unsqueeze(-1).unsqueeze(-1),
                            "ray_directions": output_ray_directions_per_view[
                                i
                            ],
                            "depth_along_ray": output_depth_along_ray_per_view[
                                i
                            ]
                            * scale_final_output.unsqueeze(-1).unsqueeze(-1),
                            "metric_scaling_factor": scale_final_output,
                ...
                output_ray_directions, output_depth_along_ray = (
                    output_dense_rep.split([3, 1], dim=-1)
                )
                ...
                output_cam_translations_per_view = (
                    output_cam_translations.chunk(num_views, dim=0)
                )
                output_cam_quats_per_view = output_cam_quats.chunk(
                    num_views, dim=0
                )
                output_pts3d_per_view = output_pts3d.chunk(num_views, dim=0)
                output_pts3d_cam_per_view = output_pts3d_cam.chunk(
                    num_views, dim=0
                )
                # Pack the output as a list of dictionaries
                res = []
                ...
                # Reshape final scene representation to (B * V, H, W, C)
                output_pts3d_cam = output_pts3d_cam.permute(
                    0, 2, 3, 1
                ).contiguous()
                # Get the predicted camera translations and quaternions
                output_cam_translations, output_cam_quats = (
                ...
                output_ray_directions = (
                    output_pts3d_cam / output_depth_along_ray
                )
            ...
            if "confidence" in self.scene_rep_type:
                ...
                output_confidences = (
                    output_confidences.permute(0, 2, 3, 1)
                    .squeeze(-1)
                    .contiguous()
                )
                # Split the predicted confidences back to their respective views
                output_confidences_per_view = output_confidences.chunk(
                    num_views, dim=0
                )
            ...
            self.geometric_input_config.update(
                {
                    ...
                    "sparse_depth_prob": 0.0,
                    "depth_scale_norm_all_prob": (
                        0.0 if use_depth_scale else 1.0
                    ),
    ...
    def infer(
        ...
        Args:
            views: List of view dictionaries. Each dict can contain:
                Required:
                - 'img': Tensor of shape (B, 3, H, W) - normalized RGB images
                ...
                - 'intrinsics': Tensor of shape (B, 3, 3) - will be converted to ray directions
                - 'ray_directions': Tensor of shape (B, H, W, 3) - ray directions in camera frame
                - 'depth_z': Tensor of shape (B, H, W, 1) - Z depth in camera frame (intrinsics or ray_directions must be provided)
                - 'camera_poses': Tensor of shape (B, 4, 4) or tuple of (quats - (B, 4), trans - (B, 3)) - can be any world frame
                - 'is_metric_scale': bool or Tensor of shape (B,) - if not provided, defaults to True
        ...
        Returns:
            List of prediction dictionaries, one per view. Each dict contains:
                - 'img_no_norm': Tensor of shape (B, H, W, 3) - denormalized rgb images
                - 'pts3d': Tensor of shape (B, H, W, 3) - predicted points in world frame
                - 'pts3d_cam': Tensor of shape (B, H, W, 3) - predicted points in camera frame
                - 'ray_directions': Tensor of shape (B, H, W, 3) - ray directions in camera frame
                - 'intrinsics': Tensor of shape (B, 3, 3) - pinhole camera intrinsics recovered from ray directions
                - 'depth_along_ray': Tensor of shape (B, H, W, 1) - depth along ray in camera frame
                - 'depth_z': Tensor of shape (B, H, W, 1) - Z depth in camera frame
                - 'cam_trans': Tensor of shape (B, 3) - camera translation in world frame
                - 'cam_quats': Tensor of shape (B, 4) - camera quaternion in world frame
                - 'camera_poses': Tensor of shape (B, 4, 4) - camera pose in world frame
                - 'metric_scaling_factor': Tensor of shape (B,) - applied metric scaling factor
                - 'mask': Tensor of shape (B, H, W, 1) - combo of non-ambiguous mask, edge mask and confidence-based mask if used
                - 'non_ambiguous_mask': Tensor of shape (B, H, W) - non-ambiguous mask
                - 'non_ambiguous_mask_logits': Tensor of shape (B, H, W) - non-ambiguous mask logits
                - 'conf': Tensor of shape (B, H, W) - confidence
