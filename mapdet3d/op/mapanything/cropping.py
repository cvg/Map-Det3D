...
from .geometry import colmap_to_opencv_intrinsics, opencv_to_colmap_intrinsics
...
def resize_with_nearest_interpolation_to_match_aspect_ratio(
    input_data, img_h, img_w
):
    """
...
def transform_boxes2d(boxes2d, scale, crop_bbox):
    """
    Transform 2D bounding boxes according to image resize and crop.

    Args:
        boxes2d (numpy.ndarray): Bounding boxes in format (N, 4) with [x1, y1, x2, y2]
        scale (float): Scale factor applied to the image
        crop_bbox (tuple): Crop bounding box as (left, top, right, bottom)

    Returns:
        numpy.ndarray: Transformed bounding boxes, clipped to image bounds
    """
    if boxes2d is None or len(boxes2d) == 0:
        return boxes2d

    boxes2d = boxes2d.copy().astype(np.float32)
    left, top, right, bottom = crop_bbox
    ...
    boxes2d[:, 0] = np.clip(boxes2d[:, 0], 0, out_w)  # x1
    boxes2d[:, 1] = np.clip(boxes2d[:, 1], 0, out_h)  # y1
    boxes2d[:, 2] = np.clip(boxes2d[:, 2], 0, out_w)  # x2
    boxes2d[:, 3] = np.clip(boxes2d[:, 3], 0, out_h)  # y2

    return boxes2d


def pad_image_and_other_optional_info(
    image,
    target_resolution,
    depthmap=None,
    camera_intrinsics=None,
    ...
    """
    Center-pad the input image and associated data to reach target resolution.

    Args:
        image (PIL.Image.Image): Input image (should be smaller than or equal to target)
        target_resolution (tuple): Target resolution as (width, height)
        depthmap (numpy.ndarray, optional): Depth map associated with the image
        camera_intrinsics (numpy.ndarray, optional): Camera intrinsics matrix
        additional_quantities (list of numpy.ndarray, optional): Additional data arrays
        pad_value (int/float/tuple): Padding value for image (default: 0 for black)
        depth_pad_value (float): Padding value for depth map (default: 0 for invalid)

    Returns:
        tuple: A tuple containing:
            - The padded image (PIL.Image.Image)
            - The padded depthmap (numpy.ndarray or None)
            - Updated camera intrinsics (numpy.ndarray or None)
            - List of padded additional quantities (list of numpy.ndarray or None)
            - pad_info (dict): Padding information for recovery
    """
    image = ImageList(image)
    w, h = image.size
    target_w, target_h = target_resolution

    # Calculate center padding amounts
    ...
    for img in image.images:
        padded_img = PIL.Image.new(img.mode, (target_w, target_h), pad_value)
        padded_img.paste(img, (pad_left, pad_top))
        padded_images.append(padded_img)
    padded_image = ImageList(padded_images)

    # Pad depthmap using np.pad
    if depthmap is not None:
        depthmap = np.pad(
            depthmap,
            ((pad_top, pad_bottom), (pad_left, pad_right)),
            mode="constant",
            constant_values=depth_pad_value,
        )

    # Pad additional_quantities
    if additional_quantities is not None:
        padded_quantities = []
        for quantity in additional_quantities:
            padded_quantities.append(
                np.pad(
                    quantity,
                    ((pad_top, pad_bottom), (pad_left, pad_right)),
                    mode="constant",
        ...
        additional_quantities = padded_quantities

    # Update intrinsics: ADD offset (opposite of crop)
    if camera_intrinsics is not None:
        camera_intrinsics = camera_intrinsics.copy()
...
def transform_boxes2d_pad(boxes2d, scale, pad_left, pad_top):
    """
    Transform 2D bounding boxes according to image resize and padding.

    Unlike cropping, padding shifts boxes in the positive direction and
    does not require clipping since all original content is preserved.

    Args:
        boxes2d (numpy.ndarray): Bounding boxes in format (N, 4) with [x1, y1, x2, y2]
        scale (float): Scale factor applied to the image
        pad_left (int): Pixels padded on the left
        pad_top (int): Pixels padded on the top
    ...
    # Apply pad offset (ADD, not subtract like in cropping)
    boxes2d[:, 0] += pad_left  # x1
    boxes2d[:, 1] += pad_top  # y1
    boxes2d[:, 2] += pad_left  # x2
    boxes2d[:, 3] += pad_top  # y2

    # No clipping needed - all original content is preserved

    return boxes2d


def recover_boxes2d_from_pad(boxes2d, pad_info, original_size=None):
    """
    Recover original bounding boxes from padded/transformed boxes.

    This reverses the transformation applied by transform_boxes2d_pad().
    Formula: original_box = (padded_box - [pad_left, pad_top, pad_left, pad_top]) / scale

    Args:
        boxes2d (numpy.ndarray): Transformed boxes in format (N, 4) with [x1, y1, x2, y2]
        pad_info (dict): Padding information dict containing:
            - 'pad_left': pixels padded on the left
            - 'pad_top': pixels padded on the top
            - 'scale': scale factor applied during preprocessing
        original_size (tuple, optional): Original image size as (width, height).
            If provided, boxes will be clipped to original bounds.

    Returns:
        numpy.ndarray: Recovered original bounding boxes
    """
    if boxes2d is None or len(boxes2d) == 0:
        return boxes2d
    ...
    # 1. Undo pad offset (reverse of += pad_left/pad_top)
    boxes2d[:, 0] -= pad_left  # x1
    boxes2d[:, 1] -= pad_top  # y1
    boxes2d[:, 2] -= pad_left  # x2
    boxes2d[:, 3] -= pad_top  # y2

    # 2. Undo scale (reverse of *= scale)
    boxes2d /= scale

    # 3. Optionally clip to original image bounds
    if original_size is not None:
        orig_w, orig_h = original_size
        boxes2d[:, 0] = np.clip(boxes2d[:, 0], 0, orig_w)  # x1
        boxes2d[:, 1] = np.clip(boxes2d[:, 1], 0, orig_h)  # y1
        boxes2d[:, 2] = np.clip(boxes2d[:, 2], 0, orig_w)  # x2
        boxes2d[:, 3] = np.clip(boxes2d[:, 3], 0, orig_h)  # y2

    return boxes2d


def pad_resize_if_necessary(
    image,
    resolution,
    depthmap=None,
    intrinsics=None,
    ...
    """
    First downsample image to fit WITHIN target resolution, then center-pad to exact size.

    This is the padding analog to crop_resize_if_necessary(). Instead of cropping
    excess content, this function pads the image to reach the target resolution,
    preserving all original image content.
    ...
    Args:
        ...
        boxes2d (numpy.ndarray, optional): 2D bounding boxes in format (N, 4) with [x1, y1, x2, y2]
        pad_value (int/float/tuple): Padding value for image (default: 0 for black)
        depth_pad_value (float): Padding value for depth map (default: 0 for invalid)

    Returns:
        tuple: A tuple containing the processed image and any provided additional data
               (depthmap, intrinsics, additional_quantities, boxes2d, pad_info)
               pad_info contains padding amounts and scale for coordinate recovery
    """
    ...
    # Calculate the scale factor - use min() to fit WITHIN target resolution
    input_resolution = np.array([original_width, original_height])
    target_resolution = np.array(resolution)
    scale_final = min(target_resolution / input_resolution)
    if scale_final >= 1:
        # Image is already smaller than target, no scaling applied
        scale_final = 1.0
        scaled_resolution = input_resolution
    else:
        # Compute the actual scaled resolution (fits within target)
        scaled_resolution = np.floor(input_resolution * scale_final).astype(
            int
        )

    # High-quality Lanczos down-scaling to fit within target
    image, depthmap, intrinsics, additional_quantities = (
        rescale_image_and_other_optional_info(
            image=image,
            output_resolution=scaled_resolution,  # Pass the pre-computed size
            depthmap=depthmap,
            camera_intrinsics=intrinsics,
            additional_quantities_to_be_resized_with_nearest=additional_quantities,
            force=True,  # Force resize to exact scaled_resolution
        )
    )

    # Safety check: ensure image doesn't exceed target resolution due to rounding
    # This can happen due to floating-point precision in scale calculations
    ...
    if w > target_w or h > target_h:
        ...
        # Need to crop the excess pixels (center crop)
        crop_left = (w - target_w) // 2 if w > target_w else 0
        crop_top = (h - target_h) // 2 if h > target_h else 0
        crop_right = crop_left + min(w, target_w)
        crop_bottom = crop_top + min(h, target_h)
        ...
        # Update scale to account for the extra crop
        # The effective scale should map original coordinates to final coordinates
        if w > target_w:
            scale_final = scale_final * target_w / w
        if h > target_h:
            scale_final = scale_final * target_h / h

    # Center-pad to exact target resolution
    image, depthmap, intrinsics, additional_quantities, pad_info = (
        pad_image_and_other_optional_info(
            image=image,
            ...
            additional_quantities=additional_quantities,
            pad_value=pad_value,
            depth_pad_value=depth_pad_value,
        )
    )

    # Transform boxes2d if provided (scale + pad offset)
    if boxes2d is not None:
        boxes2d = transform_boxes2d_pad(
            boxes2d, scale_final, pad_info["pad_left"], pad_info["pad_top"]
        )

    # Store scale in pad_info for coordinate recovery
    pad_info["scale"] = scale_final

    # Return the output (same pattern as crop_resize_if_necessary, plus pad_info)
    output = (image,)
    if depthmap is not None:
        output += (depthmap,)
...
def crop_resize_if_necessary(
    ...
    Returns:
        tuple: A tuple containing the processed image and any provided additional data
               (depthmap, intrinsics, additional_quantities, boxes2d) that have been similarly processed
    """
    # Convert image to PIL.Image.Image if necessary
    if not isinstance(image, PIL.Image.Image):
        image = PIL.Image.fromarray(image)
    ...
    # Calculate the scale factor for box transformation
    input_resolution = np.array([original_width, original_height])
    target_rescale_resolution = np.array(resolution)
    scale_final = max(target_rescale_resolution / input_resolution) + 1e-8
    if scale_final >= 1:
        # Image is already smaller than target, no scaling applied
        scale_final = 1.0
    ...
    # Transform boxes2d if provided
    if boxes2d is not None:
        boxes2d = transform_boxes2d(boxes2d, scale_final, crop_bbox)
