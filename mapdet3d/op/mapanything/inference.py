...
def validate_input_views_for_inference(
    ...
    for view_idx, view in enumerate(views):
        ...
        if missing_keys:
            raise ValueError(
                f"View {view_idx} missing required keys: {missing_keys}"
            )

        # Check for conflicting keys
        for conflict_set in CONFLICTING_KEYS:
            present_conflicts = [
                key for key in conflict_set if key in provided_keys
            ]
...
def preprocess_input_views_for_inference(
    ...
    for view_idx, view in enumerate(views):
        ...
        if "depth_z" in view:
            ...
            ray_directions_unit_plane = (
                ray_directions / ray_directions[..., 2:3]
            )
        ...
        if "camera_poses" in view:
            ...
            elif torch.is_tensor(camera_poses) and camera_poses.shape[-2:] == (
                4,
                4,
            ):
        ...
        if "ray_directions" in processed_view:
            processed_view["ray_directions_cam"] = processed_view[
                "ray_directions"
            ]
...
def postprocess_model_outputs_for_inference(
    raw_outputs: List[Dict[str, Tensor]],
    ...
    for view_idx, (raw_output, original_view) in enumerate(
        ...
        if "pts3d_cam" in processed_output:
            processed_output["depth_z"] = processed_output["pts3d_cam"][
                ..., 2:3
            ]
        ...
        if apply_mask:
            ...
            if (
                mask_edges
                and final_mask is not None
                and "pts3d" in processed_output
            ):
                # Get 3D points for edge computation
                pred_pts3d = (
                    processed_output["pts3d"].cpu().numpy()
                )  # (B, H, W, 3)
                ...
                for b in range(batch_size):
                    ...
                    if (
                        batch_final_mask.any()
                    ):  # Only compute if we have valid points
                        ...
                        edge_masks.append(
                            np.zeros_like(batch_final_mask, dtype=bool)
                        )
            ...
            if final_mask is not None:
                ...
                final_mask_torch = final_mask_torch.unsqueeze(
                    -1
                )  # (B, H, W, 1)
                ...
                for key in dense_geometry_keys:
                    if key in processed_output:
                        processed_output[key] = (
                            processed_output[key] * final_mask_torch
                        )
