...
from .cropping import crop_resize_if_necessary, pad_resize_if_necessary
from .geometry import recover_pinhole_intrinsics_from_ray_directions
...
def load_images(
    ...
    if (
        resize_mode in ["longest_side", "square", "fixed_size"]
        and size is None
    ):
        raise ValueError(
            f"Size parameter is required for resize_mode='{resize_mode}'"
        )
    ...
    for i, path in enumerate(folder_content):
        ...
        try:
            if bayer_format:
                ...
                color_bayer = cv2.imread(
                    os.path.join(root, path), cv2.IMREAD_UNCHANGED
                )
                ...
                img = exif_transpose(
                    PIL.Image.open(os.path.join(root, path))
                ).convert("RGB")
    ...
    if resize_mode == "fixed_mapping":
        ...
        if average_aspect_ratio >= 1:  # Landscape or square
            ...
            target_size = (
                size,
                round((size // patch_size) / average_aspect_ratio)
                * patch_size,
            )
        else:  # Portrait
...
def preprocess_inputs(
    ...
    pad_value: int = 0,
    depth_pad_value: float = 0,
    aspect_ratio: float | None = None,
):
    """Preprocess input_views.

    Determining optimal aspect ratio and resizing all images and multi-modal
    inputs.
    ...
    (b) Resizes all images and multi-modal inputs using crop_resize_if_necessary
        or pad_resize_if_necessary (if padding_mode=True)
    ...
    Args:
        input_views (list): List of dictionaries containing view data. Each view can contain:
            ...
            - boxes2d: 2D bounding boxes (N, 4) with [x1, y1, x2, y2] - will be resized
        ...
        resolution_set (int, optional): Resolution set to use for "fixed_mapping" mode (518 or 512). Defaults to 518.
        verbose (bool, optional): If True, print progress messages. Defaults to False.
        padding_mode (bool, optional): If True, use center padding instead of center cropping.
            This preserves all image content. Defaults to False.
        pad_value (int, optional): Padding value for image (default: 0 for black).
            Only used when padding_mode=True.
        depth_pad_value (float, optional): Padding value for depth map (default: 0 for invalid).
            Only used when padding_mode=True.
        aspect_ratio (float, optional): Desired aspect ratio (width/height). If provided,
            this ratio is used directly instead of calculating the average from input images.
            Defaults to None.

    Returns:
        list: List of processed view dictionaries with resized images and multi-modal inputs.
            When padding_mode=True, each view also contains 'pad_info' dict with padding
            amounts and scale for coordinate recovery.
    """
    # Validate resize_mode and size parameter requirements
    valid_resize_modes = [
        "fixed_mapping",
    ...
    for view_idx, view in enumerate(input_views):
        ...
        if isinstance(img, torch.Tensor):
            ...
            raise ValueError(
                f"Unsupported image type in view {view_idx}: {type(img)}"
            )
    ...
    if aspect_ratio is not None:
        average_aspect_ratio = aspect_ratio
        if verbose:
            print(f"Using provided aspect ratio: {average_aspect_ratio:.3f}")
    else:
        average_aspect_ratio = sum(aspect_ratios) / len(aspect_ratios)
        if verbose:
            print(
    ...
    for view_idx, view in enumerate(input_views):
        ...
        if has_intrinsics:
            intrinsics = to_numpy(
                view["intrinsics"], (3, 3), "intrinsics", view_idx
            )
        ...
        if has_ray_directions:
            ...
            recovered_intrinsics = (
                recover_pinhole_intrinsics_from_ray_directions(ray_dirs_torch)
            )
        ...
        boxes2d = None
        if "boxes2d" in view:
            boxes2d = to_numpy(view["boxes2d"], None, "boxes2d", view_idx)
            if boxes2d.ndim != 2 or boxes2d.shape[1] != 4:
                raise ValueError(
                    f"Expected shape (N, 4) for boxes2d in view {view_idx}, got {boxes2d.shape}"
                )

        # Process all inputs with crop or pad based on padding_mode
        if padding_mode:
            results = pad_resize_if_necessary(
                image=img,
            ...
            # pad_resize_if_necessary always returns pad_info as the last element
            pad_info = results[-1]
            results = results[:-1]
        else:
        ...
        if depthmap is not None:
            processed_view["depth_z"] = torch.from_numpy(results[result_idx])[
                None
            ]
            result_idx += 1
        ...
        if intrinsics is not None:
            processed_view["intrinsics"] = torch.from_numpy(
                results[result_idx]
            )[None]
            result_idx += 1

        # boxes2d is next if provided - add batch dimension
        if boxes2d is not None:
            processed_view["boxes2d"] = torch.from_numpy(results[result_idx])[
                None
            ]
            result_idx += 1

        # Add pad_info if padding mode was used
        if pad_info is not None:
            processed_view["pad_info"] = pad_info
        ...
        if "camera_poses" in view:
            ...
            if isinstance(camera_poses, tuple):
                ...
                if isinstance(camera_poses, torch.Tensor):
                    ...
                    processed_view["camera_poses"] = torch.from_numpy(
                        camera_poses
                    )[None]
                else:
                    raise ValueError(
        ...
        if verbose:
            print(
                f"Processed view {view_idx} with keys: {list(processed_view.keys())}"
            )
