...
from .misc import invalid_to_zeros
...
def project_pts3d_to_image(pts3d, intrinsics, return_z_dim):
    ...
    projected_pts2d = ein.einsum(
        intrinsics, pts3d, "b i k, b h w k -> b h w i"
    )
    projected_pts2d[..., :2] /= (
        projected_pts2d[..., 2].unsqueeze(-1).clamp(min=1e-6)
    )
...
def recover_pinhole_intrinsics_from_ray_directions(
    ...
    if is_high_res or use_geometric_calculation:
        ...
        center_rays = ray_directions[
            :, center_h, center_w, :
        ].clone()  # [B, 3]
        left_rays = ray_directions[:, center_h, quarter_w, :].clone()  # [B, 3]
        right_rays = ray_directions[
            :, center_h, three_quarter_w, :
        ].clone()  # [B, 3]
        top_rays = ray_directions[:, quarter_h, center_w, :].clone()  # [B, 3]
        bottom_rays = ray_directions[
            :, three_quarter_h, center_w, :
        ].clone()  # [B, 3]
        ...
        fx_left = (quarter_w - center_w) / (
            left_rays[:, 0] - center_rays[:, 0]
        )
        fx_right = (three_quarter_w - center_w) / (
            right_rays[:, 0] - center_rays[:, 0]
        )
...
def convert_raymap_z_depth_quats_to_pointmap(
    ray_origins, ray_directions, depth, quats
):
    """
...
def rotation_matrix_to_quaternion(matrix: Tensor) -> Tensor:
    """
...
def convert_ray_dirs_depth_along_ray_pose_trans_quats_to_pointmap(
    ...
    pts3d_homo = torch.cat(
        [pts3d_local, torch.ones_like(pts3d_local[..., :1])], dim=-1
    )
...
def xy_grid(
    ...
    **arange_kw,
    ...
    Returns:
        numpy.ndarray or Tensor: Coordinate grid where:
...
def geotrf(Trf, pts, ncol=None, norm=False):
    ...
    # Optimized code
    if (
        isinstance(Trf, Tensor)
        and isinstance(pts, Tensor)
        and Trf.ndim == 3
        and pts.ndim == 4
    ):
        d = pts.shape[3]
...
def closed_form_pose_inverse(
    ...
    # Validate shapes
    if pose_matrices.shape[-2:] != (4, 4) and pose_matrices.shape[-2:] != (
        3,
        4,
    ):
        raise ValueError(
    ...
    if is_numpy:
        ...
        inverted_matrix = torch.eye(4, 4)[None].repeat(
            len(rotation_matrices), 1, 1
        )
...
def relative_pose_transformation(trans_01, trans_02):
    ...
    Arguments:
        trans_01 (Tensor): reference transformation tensor of shape
         :math:`(N, 4, 4)` or :math:`(4, 4)`.
        trans_02 (Tensor): destination transformation tensor of shape
         :math:`(N, 4, 4)` or :math:`(4, 4)`.
    ...
    Returns:
        Tensor: the relative transformation between the transformations.
    ...
    if not torch.is_tensor(trans_01):
        raise TypeError(
            "Input trans_01 type is not a Tensor. Got {}".format(
                type(trans_01)
            )
        )
    if not torch.is_tensor(trans_02):
        raise TypeError(
            "Input trans_02 type is not a Tensor. Got {}".format(
                type(trans_02)
            )
        )
    if trans_01.dim() not in (2, 3) and trans_01.shape[-2:] == (4, 4):
        raise ValueError(
            "Input must be a of the shape Nx4x4 or 4x4. Got {}".format(
                trans_01.shape
            )
        )
...
def depthmap_to_camera_coordinates(
    depthmap, camera_intrinsics, pseudo_focal=None
):
    """
    Args:
        - depthmap (HxW array):
...
def depthmap_to_absolute_camera_coordinates(
    ...
    if camera_pose is not None:
        ...
        X_world = (
            np.einsum("ik, vuk -> vui", R_cam2world, X_cam)
            + t_cam2world[None, None, :]
        )
...
def get_absolute_pointmaps_and_rays_info(
    ...
    ray_dirs_cam_on_unit_plane = np.stack(
        (x_cam, y_cam, z_cam), axis=-1
    ).astype(np.float32)
...
def normalize_depth_using_non_zero_pixels(depth, return_norm_factor=False):
    ...
    Args:
        depth (Tensor): Depth tensor of size [B, H, W, 1].
    Returns:
        normalized_depth (Tensor): Normalized depth tensor.
        norm_factor (Tensor): Norm factor tensor of size B.
    """
...
def normalize_pose_translations(pose_translations, return_norm_factor=False):
    ...
    Args:
        pose_translations (Tensor): Pose translations tensor of size [B, V, 3]. B is the batch size, V is the number of views.
    Returns:
        normalized_pose_translations (Tensor): Normalized pose translations tensor of size [B, V, 3].
        norm_factor (Tensor): Norm factor tensor of size B.
    """
    ...
    sum_of_all_views_pose_translations = pose_translations_dis.sum(
        dim=1
    )  # [B]
...
def apply_log_to_norm(input_data):
    ...
    Args:
        input_data (Tensor): The input tensor to be normalized and transformed.

    Returns:
        Tensor: The transformed tensor after normalization and logarithmic scaling.
    """
...
def angle_diff_vec3(v1, v2, eps=1e-12):
    ...
    Args:
        v1: Tensor of shape (..., 3)
        v2: Tensor of shape (..., 3)
        eps: Small epsilon value for numerical stability

    Returns:
        Tensor: Angle differences in radians
    """
...
def sliding_window_1d(
    ...
    x_sliding = np.lib.stride_tricks.as_strided(
