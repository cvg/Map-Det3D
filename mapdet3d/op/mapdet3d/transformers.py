...
from uniception.models.utils.transformer_blocks import (
    CrossAttentionBlock,
    Mlp,
    SelfAttentionBlock,
)
...
class MultiViewAlternatingAttentionTransformer(UniCeptionInfoSharingBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.distinguish_ref_and_non_ref_views = (
            distinguish_ref_and_non_ref_views
        )
        self.use_pe_for_non_reference_views = use_pe_for_non_reference_views
        self.max_num_views_for_pe = max_num_views_for_pe
        self.use_rand_idx_pe_for_non_reference_views = (
            use_rand_idx_pe_for_non_reference_views
        )
        self.depth = depth
        self.dim = dim
        ...
        self.base_token_count_for_entropy_scaling = (
            base_token_count_for_entropy_scaling
        )
        ...
        if self.input_embed_dim != self.dim:
            self.proj_embed = nn.Linear(
                self.input_embed_dim, self.dim, bias=True
            )
        else:
            ...
            if self.use_pe_for_non_reference_views:
                ...
                self.register_buffer(
                    ...
                    self._get_sinusoid_encoding_table(
                        self.max_num_views_for_pe, self.dim, 10000
                    ),
                )
            else:
        ...
        if self.gradient_checkpointing:
            for i, block in enumerate(self.self_attention_blocks):
                self.self_attention_blocks[i] = (
                    self.wrap_module_with_gradient_checkpointing(block)
                )
    ...
    def forward(
        ...
        assert all(
            view_features.shape[1] == self.input_embed_dim
            for view_features in model_input.features
        ...
        multi_view_features = multi_view_features.permute(
            0, 1, 3, 4, 2
        )  # (N, V, H, W, C)
        ...
        if self.custom_positional_encoding is not None:
            multi_view_positions = [
                self.position_getter(
                    batch_size, height, width, multi_view_features.device
                )
            ] * num_of_views  # List of length V, where each tensor is (N, H * W, C)
            multi_view_positions = torch.cat(
                multi_view_positions, dim=1
            )  # (N, V * H * W, C)
        else:
        ...
        if model_input.additional_input_tokens is not None:
            ...
            additional_tokens_positions = [
                None
            ] * model_input.additional_input_tokens.shape[1]
            multi_view_positions = (
                multi_view_positions + additional_tokens_positions
            )
        ...
        if self.distinguish_ref_and_non_ref_views:
            ...
            ref_view_pe = ref_view_pe.repeat(
                batch_size, num_of_tokens_per_view, 1
            )
            ref_view_features = multi_view_features[
                :, :num_of_tokens_per_view, :
            ]
        ...
        if (
            self.distinguish_ref_and_non_ref_views
            and self.use_pe_for_non_reference_views
        ):
            ...
            non_ref_view_pe = (
                self.view_pos_table[non_ref_view_pe_indices].clone().detach()
            )
            non_ref_view_pe = non_ref_view_pe.reshape(
                (1, num_of_views - 1, self.dim)
            )
            non_ref_view_pe = non_ref_view_pe.repeat_interleave(
                num_of_tokens_per_view, dim=1
            )
        ...
        if model_input.additional_input_tokens is not None:
            ...
            additional_features = multi_view_features[
                :, num_of_views * num_of_tokens_per_view :, :
            ]
            multi_view_features = torch.cat(
                [
                    ref_view_features,
                    non_ref_view_features,
                    additional_features,
                ],
                dim=1,
            )
            ...
            multi_view_features = torch.cat(
                [ref_view_features, non_ref_view_features], dim=1
            )
        ...
        for depth_idx in range(self.depth):
            if depth_idx % 2 == 0:
                ...
                multi_view_features = self.self_attention_blocks[depth_idx](
                    multi_view_features, multi_view_positions
                )
            else:
                ...
                if model_input.additional_input_tokens is not None:
                    ...
                    if self.custom_positional_encoding is not None:
                        additional_positions = multi_view_positions[
                            :, num_of_views * num_of_tokens_per_view :, :
                        ]
                        multi_view_positions = multi_view_positions[
                            :, : num_of_views * num_of_tokens_per_view, :
                        ]
                ...
                if additional_features is not None:
                    multi_view_features = torch.cat(
                        [multi_view_features, additional_features], dim=1
                    )
                    # Reattach positions for additional tokens if they exist
                    if additional_positions is not None:
                        multi_view_positions = torch.cat(
                            [multi_view_positions, additional_positions], dim=1
                        )
        ...
        view_features = view_features.permute(
            0, 1, 4, 2, 3
        ).contiguous()  # (N, V, C, H, W)

        # Split the output multi-view features into separate views
        view_features = view_features.split(1, dim=1)
        view_features = [
            output_view_features.squeeze(dim=1)
            for output_view_features in view_features
        ]

        # Extract and return additional token features if provided
        if model_input.additional_input_tokens is not None:

            additional_token_features = output_multi_view_features[
                :, num_of_views * num_of_tokens_per_view :, :
            ]
...
class MultiViewAlternatingAttentionTransformerIFR(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Initialize the cross-attention blocks which ingest all views at once
        ...
        # Initialize the projection layer for query embeddings
        if self.input_embed_dim != self.dim:
            self.query_proj_embed = nn.Linear(
                self.input_embed_dim, self.dim, bias=True
            )
        else:
            self.query_proj_embed = nn.Identity()

    def _condition_query(
        self,
        cross_attention_block: nn.Module,
        self_attention_block: nn.Module,
        query: Tensor,
        multi_view_features: Tensor,
    ) -> Tensor:
        "Condition the query for cross-attention block"
        # query = query + cross_attention_block.drop_path1(
        #     cross_attention_block.ls1(
        #         cross_attention_block.attn(
        #             cross_attention_block.norm1(query), None
        #         )
        #     )
        # )
        # y_ = cross_attention_block.norm_y(multi_view_features.detach())
        # query = query + cross_attention_block.drop_path2(
        #     cross_attention_block.ls2(
        #         cross_attention_block.cross_attn(
        #             cross_attention_block.norm2(query), y_, y_, None, None
        #         )
        #     )
        # )

        # # Hacky way to do cross-attention with self-attention block
        # query = query + cross_attention_block.drop_path3(
        #     cross_attention_block.ls3(
        #         self_attention_block.mlp(cross_attention_block.norm3(query))
        #     )
        # )
        return cross_attention_block(query, multi_view_features.detach())
    ...
    def forward(
        ...
        if query is not None:
            query = self.query_proj_embed(query.permute(0, 2, 1).contiguous())
        ...
        for depth_idx in range(self.depth):
            if depth_idx % 2 == 0:
                ...
                # Condition the query if provided
                if query is not None:
                    query = self._condition_query(
                        self.cross_attention_blocks[depth_idx],
                        self.self_attention_blocks[depth_idx],
                        query,
                        multi_view_features,
                    )
            else:
        ...
        for idx in range(len(intermediate_multi_view_features)):
            ...
            if model_input.additional_input_tokens is not None:
                ...
                additional_token_features = current_features[
                    :, num_of_views * num_of_tokens_per_view :, :
                ]
                additional_token_features = additional_token_features.permute(
                    0, 2, 1
            ...
            current_features = [
                ...
                for intermediate_view_features in current_features
            ]
...
def test_reshape_for_frame_attention():
    ...
    # Apply reshape
    reshaped = x.reshape(
        batch_size * num_of_views, num_of_tokens_per_view, dim
    ).contiguous()

    # Verify shape
    assert reshaped.shape == (
        batch_size * num_of_views,
        num_of_tokens_per_view,
        dim,
    )
        ...
        for v in range(num_of_views):
            for h in range(height):
                for w in range(width):
                    ...
                    assert torch.all(
                        reshaped[batch_view_idx, token_idx] == expected
                    )

    # Verify reshape back works
    back_to_original = reshaped.reshape(
        batch_size, num_of_views * num_of_tokens_per_view, dim
    )
...
if __name__ == "__main__":
    ...
    for num_views in [2, 3, 4]:
        print(
            f"Testing MultiViewAlternatingAttentionTransformer with {num_views} views ..."
        )
        ...
        assert all(
            f.shape == (1, model.dim, 14, 14) for f in model_output.features
        )
    ...
    print(
        "All multi-view alternating-attention transformers initialized and tested successfully!"
    )
    ...
    model_intermediate_feature_returner = (
    ...
    assert all(
        isinstance(intermediate, MultiViewTransformerOutput)
    ...
    print(
        "Testing MultiViewAlternatingAttentionTransformer with additional input tokens ..."
    )
    model = MultiViewAlternatingAttentionTransformer(
        name="MV-AAT",
        input_embed_dim=1024,
    ...
    model_input = MultiViewTransformerInput(
        features=model_input, additional_input_tokens=additional_tokens
    )
    model_output = model(model_input)
    assert len(model_output.features) == num_views
    assert all(
    ...
    print(
        "Testing MultiViewAlternatingAttentionTransformerIFR with additional input tokens ..."
    )
    ...
    assert all(
        intermediate.additional_token_features is not None
        for intermediate in output[1]
    )
    assert all(
        intermediate.additional_token_features.shape
        == (1, model_ifr.dim, num_additional_tokens)
        for intermediate in output[1]
    )
